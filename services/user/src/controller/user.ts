...
export const updateUserProfile = TryCatch(async (req: AuthenticatedRequest, res) => {
    ...
    if (!user) {
        throw new ErrorHandler(401, "Authcation Required");
    }
...
export const updateProfilePic = TryCatch(async (req: AuthenticatedRequest, res) => {
    ...
    const file = req.file;
    if (!file) {
        throw new ErrorHandler(400, " No Image file provied");
    ...
    if (!fileBuffer || !fileBuffer.content) {
        throw new ErrorHandler(500, "FAiled to genarte biffer");
    }
...
export const updateResume = TryCatch(async (req: AuthenticatedRequest, res) => {
    ...
    const file = req.file;
    if (!file) {
        throw new ErrorHandler(400, " No Resume pdf file provied");
    ...
    res.json({
        message: "Resuem updated successfully",
        updatedUser
    })
...
export const addSkillToUser = TryCatch(async (req: AuthenticatedRequest, res) => {
    ...
    console.log("skillsNAme : ", skillName)

    if (!skillName || skillName.trim() === "") {
        throw new ErrorHandler(400, "Please Provide the skills");
    }
    ...
    if (!wasSkillAdded) {
        return res.status(200).json({
            message: "user already possed this skills",
        })
    }
...
export const deleteSkillFromUser = TryCatch(async (req: AuthenticatedRequest, res) => {
    ...
    if (result.length === 0) {
        throw new ErrorHandler(400, `Skill ${skillName.trim()} was  not found`)
    }
...
export const applyForJob = TryCatch(async (req: AuthenticatedRequest, res) => {
    ...
    if (!resume) {
        throw new ErrorHandler(400, "No need to add resume in your profile to apply")
    }
    ...
    try {
        [newApplciation] = await sql`INSERT INTO applications (job_id , applicant_id , applicant_email,resume ,subscribed) VALUES (${job_id},${applicant_id},${user?.email} ,${resume},${isSubscribed})  RETURNING * `
    } catch (error: any) {
        if (error.code === "23505") {
            throw error;
        }
...
export const getAllApplication = TryCatch(async (req: AuthenticatedRequest, res) => {
    ...
    SELECT a.* , j.title AS job_title,j.salary AS job_salary,j.location AS job_location FROM applications a JOIN jobs j ON a.job_id  = j.job_id WHERE a.applicant_id = ${userId}`;
